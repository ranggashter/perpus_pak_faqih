 ...
 * ============================================
 * FILE: tambah.php
 * FUNGSI: Form + proses tambah buku
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($kode_buku === "" || $judul === "" || $penulis === "" ||
        ...
        $foto = uploadFotoBuku($_FILES['foto'] ?? [], $error);

        if ($foto === false) {
            $error = $error;
        } else {
        // --- Cek kode_buku duplikat ---
        $cek = mysqli_prepare($conn, "SELECT id FROM books WHERE kode_buku = ?");
        mysqli_stmt_bind_param($cek, "s", $kode_buku);
        mysqli_stmt_execute($cek);
        mysqli_stmt_store_result($cek);

        if (mysqli_stmt_num_rows($cek) > 0) {
            $error = "Kode buku <strong>$kode_buku</strong> sudah dipakai!";
        } else {
            // --- Insert data ---
            $stmt = mysqli_prepare($conn,
                "INSERT INTO books (kode_buku, judul, penulis, kategori, tahun_terbit, penerbit, foto)
                 VALUES (?, ?, ?, ?, ?, ?, ?)");
...
<body>
    ...
    <div class="navbar">
        ...
        <div class="menu">
            ...
            <a href="tambah.php" class="active">Tambah Buku</a>
    ...
    <div class="container">
        <div class="card">
            <h2>Tambah Buku</h2>
            <p class="subtitle">Lengkapi data buku di bawah ini. Field bertanda * wajib diisi.</p>
            ...
            <form method="POST" enctype="multipart/form-data">
                <div class="form-group">
                    ...
                    <input type="text" name="kode_buku" placeholder="Contoh: BK011"
                           value="<?= htmlspecialchars($_POST['kode_buku'] ?? '') ?>" required maxlength="20">
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="penulis" placeholder="Nama penulis"
                           value="<?= htmlspecialchars($_POST['penulis'] ?? '') ?>" required maxlength="100">
                </div>

                <div class="form-group">
                    <label>Kategori *</label>
                    <input type="text" name="kategori" placeholder="Contoh: Novel, Teknologi"
                           value="<?= htmlspecialchars($_POST['kategori'] ?? '') ?>" required maxlength="50">
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="tahun_terbit" placeholder="Contoh: 2020"
                           ...
                           value="<?= htmlspecialchars($_POST['tahun_terbit'] ?? '') ?>" required>
                </div>

                <div class="form-group">
                    <label>Penerbit *</label>
                    <input type="text" name="penerbit" placeholder="Nama penerbit"
                           value="<?= htmlspecialchars($_POST['penerbit'] ?? '') ?>" required maxlength="100">
                </div>
