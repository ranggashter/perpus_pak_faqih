...
/**
 * ============================================
 * FILE: user_tambah.php
 * FUNGSI: Form tambah pengguna baru (admin only)
 * ============================================
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nama === '' || $username === '' || $password === '' || $confirmPassword === '') {
        ...
        if (mysqli_stmt_num_rows($cek) > 0) {
            $error = 'Username <strong>' . htmlspecialchars($username) . '</strong> sudah dipakai!';
        } else {
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                $_POST = [];
                $nama = $username = $password = $confirmPassword = '';
                $role = 'petugas';
            } else {
...
<body>
    ...
    <div class="container">
        <div class="card">
            <h2>Tambah User</h2>
            <p class="subtitle">Buat akun pengguna baru. Field bertanda * wajib diisi.</p>
            ...
            <form method="POST">
                ...
                <div class="form-group">
                    <label>Role *</label>
                    <select name="role">
                        <option value="admin" <?= ($_POST['role'] ?? 'petugas') === 'admin' ? 'selected' : '' ?>>Admin</option>
                        <option value="petugas" <?= ($_POST['role'] ?? 'petugas') === 'petugas' ? 'selected' : '' ?>>Petugas</option>
                    </select>
                </div>
